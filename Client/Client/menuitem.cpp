...
hgeGUIMenuItem::hgeGUIMenuItem(int _id, hgeFont *_fnt, HEFFECT _snd, float _x, float _y, float _delay, char *_title, DWORD _clr, DWORD _sclr)
{
	float w;
	...
	color.SetHWColor(clr);
	shadow.SetHWColor(sclr);
	offset=0.0f;
	timer=-1.0f;
	timer2=-1.0f;
...
{
	fnt->SetColor(/*shadow.GetHWColor()*/ARGB(200, 0, 0, 0));
...
void hgeGUIMenuItem::Enter()
{
	...
	tcolor2.SetHWColor(clr);
	dcolor2=tcolor2-scolor2;

	sshadow.SetHWColor(sclr);
...
void hgeGUIMenuItem::Focus(bool bFocused)
{
	...
	{
		hge->Effect_Play(snd);
		scolor.SetHWColor(clr);
		tcolor.SetHWColor(sclr);
		soffset=0;
		doffset=4;
	}
	else
	{
		scolor.SetHWColor(sclr);
		tcolor.SetHWColor(clr);
		soffset=4;
		doffset=-4;
	}
